...
describe("Testing create ticket feature", () => {
    ...
    it("Not able to create a ticket -Error", async() => {
        ...
        const ticketSpy = jest.spyOn(Ticket, 'create').mockImplementation(cb => cb(new Error("Error while creating"), null));
...
describe("Testing update ticket feature", () => {
    ...
    it('testing that the user ticket is successfully updated', async(done) => {
        const req = mockRequest();
        const res = mockResponse();
        req.params = {
            userId: 1
        ...
        const ticketSpy = jest.spyOn(Ticket, 'findOne').mockReturnValue(Promise.resolve(createdTicketBody));
